    ...
    await axios.post(
      `https://graph.facebook.com/${version}/${phone_no_id}/messages?access_token=${token}`,
      {
        ...
        interactive: {
          ...
          body: {
            text: `Your ${travelMode} will depart on ${departureTime1} and arrive on ${arrivalTime1}. The cost of this journey is ${amount}. Please see other available time options for this journey.`,
          },
          footer: {
            text: "Thank you for using Online Transport!",
          },
...
// ask for email
export async function sendForEmail(to: string, msg: string): Promise<boolean> {
  try {
    // Make the Axios request to send the message
    await axios.post(
      `https://graph.facebook.com/${version}/${phone_no_id}/messages?access_token=${token}`,
...
export async function sendRegSuccess(
  to: string,
  msg: string
): Promise<boolean> {
  try {
...
// ask for full name
export async function sendFullName(to: string, msg: string): Promise<boolean> {
  try {
    // Make the Axios request to send the message
    await axios.post(
      `https://graph.facebook.com/${version}/${phone_no_id}/messages?access_token=${token}`,
...
export async function sendRegSuccessfulMsg(
  to: string,
  msg: string
): Promise<boolean> {
  try {
...
export async function sendInteractiveMessage2(
  to: string,
  body: string,
  buttons: any[]
): Promise<boolean> {
